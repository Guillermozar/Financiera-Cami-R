...
export default function SecuritySection() {
  ...
  const safetyBgClasses = [
    "bg-white/5",
    "bg-white/5",
    "bg-red-500/10"
  ];

  // Retrieve translation arrays
  const safetyFeatures = t('securitySection.features') || [];
  const certifiedSpecs = t('securitySection.specs') || [];
        ...
        <div className="space-y-10">
          ...
          <div className="space-y-2" role="list" aria-label="Características de seguridad">
            {safetyFeatures.map((item, idx) => (
              ...
              >
                <div className={`shrink-0 w-14 h-14 ${safetyBgClasses[idx] || 'bg-white/5'} rounded-2xl flex items-center justify-center transition-transform group-hover:scale-105`} aria-hidden="true">
                  {safetyIcons[idx]}
                </div>
                <div>
